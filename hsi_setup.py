...
def train_options(parser):
    ...
    parser.add_argument('--no-cuda', action='store_false', help='disable cuda?')
...
class Engine(object):
    ...
    def __setup(self):
        ...
        #input_shape = self.net.fc.in_features
        #print("The Netwrok", input_shape, "\n\n")
        ...
        if self.opt.resume:
            # Load checkpoint.
            print("self.opt.no_ropt: ", self.opt.no_ropt)
...
#        print(self.net.flops([64,64]))
    ...
    def __step(self, train, inputs, targets,sigma=None):        
        ...
        if self.get_net().bandwise:
            ...
            #print(inputs[:, 0:31].shape)
            #print(inputs.shape)

            #b = torch.randn(1, 34, 512, 614)

            #inputs = torch.cat((inputs, b), dim=1)[:, :, 0: 100, 0:100]

            #print(targets.shape)

            #targets = torch.cat((inputs, b[:, :, 0: 100, 0:100]), dim=1)

            #print(inputs.shape)

            #fig, axes = plt.subplots(2,2, figsize=(14,7))
            #axes[0, 0].pcolormesh(inputs[0,10])
            #plt.show()
           ...
           # outputs = torch.clamp(outputs, 0, 1)
            ...
            #     loss = self.criterion(outputs[:,0,...], targets[:,0,...])
            # else:
            #targets = torch.cat((targets, b[:, :, 0: 100, 0:100]), dim=1)
    ...
    def load(self, resumePath=None, load_opt=True):
        ...
        checkpoint = torch.load(resumePath, map_location=torch.device('cpu'))
        # if load_opt:
    ...
    def test_numpy(self, numpy_images_input, numpy_images_target):
        self.net.eval()

        outputs, loss_data, _ = self.__step(False, numpy_images_input, numpy_images_target)

        return (outputs, loss_data)
        #for i in range(len(numpy_images_input)):
        #    inputs, targets = inputs.to(self.device), targets.to(self.device) 
    ...
    def test(self, valid_loader, filen):
        ...
        with torch.no_grad():
            #for one, two in enumerate(valid_loader):
              #print("valid_loader: ", one, "\n")
            for batch_idx, (inputs, targets) in enumerate(valid_loader):
                #print("batch_idx: ", batch_idx ,"\n")
                #print("inputs: ", inputs ,"\n")
                #print("targets: ", targets ,"\n")
                targets = targets[:, :, 0: 100, 0:100]
                if not self.opt.no_cuda:
                    inputs, targets = inputs.to(self.device), targets.to(self.device) 
                ...
                print("Outputs: ", outputs[0,10].shape, "\n")
                axes[0, 0].pcolormesh(outputs[0,10])
                axes[0, 1].pcolormesh(targets[0,10])
                plt.show()
