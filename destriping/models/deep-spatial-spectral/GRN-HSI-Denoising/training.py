...
tf.app.flags.DEFINE_integer(
    "num_patches", 500, """number of patches in each h5 file."""
)
tf.app.flags.DEFINE_float("learning_rate", 0.0005, """learning rate.""")
tf.app.flags.DEFINE_integer("epoch", 10, """epoch.""")
tf.app.flags.DEFINE_integer("batch_size", 32, """Batch size.""")
tf.app.flags.DEFINE_integer("num_channels", 31, """Number of channels of the input.""")
...
tf.app.flags.DEFINE_string(
    "save_model_path", "./model/ICVL_stage1/", "The path of saving model"
)
...
if __name__ == "__main__":
    ...
    with tf.Session(config=config) as sess:
        ...
        validation_h5_data, validation_h5_label = read_data(
        ...
        if tf.train.get_checkpoint_state(save_path):  # load previous trained model
            ...
            training_error = io.loadmat(save_path + "training_error.mat")[
                "training_error"
            ]
        ...
        for j in range(start_point, epoch):  # epoch
            ...
            for num in range(FLAGS.num_h5_file):  # h5 files
                ...
                data_size = int(
                    FLAGS.num_patches / FLAGS.batch_size
                )  # the number of batch
                for i in range(data_size):
                    rand_index = np.arange(
                        int(i * FLAGS.batch_size), int((i + 1) * FLAGS.batch_size)
                    )  # batch
                    ...
                    _, lossvalue = sess.run(
                        [g_optim, loss],
                        feed_dict={images: batch_data, labels: batch_label, lr: lr_},
                    )
            ...
            Validation_Loss = sess.run(
                loss, feed_dict={images: validation_data, labels: validation_label}
            )  # validation loss
            end = time.time()
            print(
                "%d epoch is finished, learning rate = %.4f, Training_Loss = %.4f, Validation_Loss = %.4f, runtime = %.1f s"
