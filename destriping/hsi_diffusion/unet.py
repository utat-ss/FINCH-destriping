"""Code from https://github.com/openai/guided-diffusion.
"""
...
class UNetModel(nn.Module):
    ...
    def forward(self, x, timesteps, y=None, scale=None):
        """
        ...
        if self.condition_on_scale and scale is not None:
            assert scale.shape == (x.shape[0],)
            emb = emb + self.scale_emb(
                timestep_embedding(scale, self.model_channels, 1)
            )
        ...
        for module in self.output_blocks:
            ...
            h = module(h, emb)
        h = h.type(x.dtype)
        return UNet3DOutput(sample=self.out(h))
