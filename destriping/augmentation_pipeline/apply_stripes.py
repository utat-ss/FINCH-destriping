...
def add_stripes(datacube, configs=None):
    ...
    striped_data = copy.deepcopy(datacube)
...
def _gaussian_stripe(data, configs):
    ...
    if stripe_intensity < 0:
        ...
        stripe_intensity = np.random.uniform(
            stripe_intensity, stripe_intensity + 0.1, 1
        )
    ...
    for i in range(data.shape[2]):
        ...
        noise = np.round(abs(np.random.normal(mean, std_dev, len(col_lines)))).astype(
            "<u2"
        )
...
def _choose_slices(data, i, col_lines, noise):
    ...
    for j, col in enumerate(col_lines):
        ...
        # get random indices
        frags = np.array(
            np.random.choice(fragment_starts.shape[0], num_frag_noise, replace=False)
        )
...
def _select_lines(dims, configs):
    ...
    if clusters:
        ...
        cluster_cols = np.round(np.random.uniform(0, dims[1] - 1, num_clusters)).astype(
            np.int32
        )
        ...
        for col in cluster_cols:
            ...
            if (stripe_frequency > 1) or (stripe_frequency < 0):
                num_stripes = np.round(np.random.uniform(0, dims[1], size=1)).astype(
                    np.int32
                )
            else:
            ...
            cluster = np.round(np.random.uniform(start, end, num_stripes)).astype(
                np.int32
            )
        ...
        cols_striped.update(
            np.round(
                np.random.uniform(0, dims[1] - 1, num_lines - len(cols_striped))
            ).astype(np.int32)
        )
        ...
        num_lines = np.round(np.random.uniform(0, (dims[1] - 1) // 2, 1)).astype(
            np.int32
        )
    else:
        num_lines = int((dims[1]) * stripe_frequency)
        cols_striped = np.round(
            np.random.uniform(0, dims[1] - 1, num_lines - 1)
        ).astype(np.int32)
    # print(cols_striped)
    return cols_striped
